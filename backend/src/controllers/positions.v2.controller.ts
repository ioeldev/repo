...
import { calculatePnl as calculatePnlUtil, calculatePnlWithFees, calculateUnrealizedPnl } from "../utils/pnl";
import { liquidationMonitor } from "../services/liquidation-monitor.service";
import axios from "axios";

/**
 * Calculate isolated margin liquidation price
 * Simple formula: entryPrice * (1 +/- 0.95 / leverage)
 * The 0.95 factor accounts for fees and provides a safety buffer
 */
const calculateIsolatedLiquidationPrice = (type: "buy" | "sell", entryPrice: number, leverage: number): number => {
    if (leverage <= 1) {
        // No liquidation for 1x leverage (no margin used)
        return type === "buy" ? 0 : Number.MAX_SAFE_INTEGER;
    }

    if (type === "buy") {
        // For BUY: liquidation when price drops
        return entryPrice * (1 - 0.95 / leverage);
    } else {
        // For SELL: liquidation when price rises
        return entryPrice * (1 + 0.95 / leverage);
    }
};
...
const calculateFees = (pnl: number, feePercentage: number | undefined): number => {
...
const sendPositionEmail = async (emailType: "open" | "close", user: any, positionData: any) => {
    try {
    ...
    try {
        ...
        const positionsWithPnL = positions.map((position: any) => {
            ...
            if (position.status === "closed" && position.exit_price) {
                ...
                pnl = position.fees ? parseFloat((rawPnl - position.fees).toFixed(2)) : parseFloat(rawPnl.toFixed(2));
            }
...
export const getMyPositionStats = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const btcPriceResponse = await axios.get(`https://api.mexc.com/api/v3/ticker/price?symbol=BTCUSDT`);
        const btcPrice = parseFloat(parseFloat(btcPriceResponse.data.price).toFixed(2));
        ...
        positions.forEach((position) => {
            ...
            const currencyStats = stats[currency as keyof typeof stats] || { pnl: 0, openCount: 0, closedCount: 0 };
            ...
            if (position.status === "open") {
                ...
                const netPnl = position.fees ? (pnl > 0 ? pnl - position.fees : pnl) : pnl;
        ...
        const balances = user.balances.reduce((acc, balance) => {
            acc[balance.symbol.toLowerCase()] = parseFloat(balance.balance.toFixed(2));
            return acc;
                            ...
                            in: {
                                $cond: {
                                    ...
                                    then: {
                                        ...
                                        $multiply: [{ $toDouble: "$quantity" }, { $multiply: ["$$priceDiff", -1] }],
                                    },
                                },
                            },
                        },
...
export const createPosition = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const currencyBalance = userBalance?.balances.find((b) => b.symbol === base_currency);
        ...
        const sameTypePositions = existingPositions.filter((pos) => pos.type === type);
        if (sameTypePositions.length > 0) {
            const differentLeveragePosition = sameTypePositions.find((pos) => pos.leverage !== leverage);
        ...
        // Check if we have an opposing position that requires reversal
        const opposingPositions = existingPositions.filter((pos) => pos.type !== type);
        ...
        if (opposingPositions.length > 0 && confirmReversal) {
            ...
            for (const opposingPosition of opposingPositions) {
                ...
                const collateralToReturn = opposingPosition.base_currency_amount * closingRatio;
                ...
                if (remainingQty > 0) {
                    ...
                    const remainingCollateral = opposingPosition.base_currency_amount * (1 - closingRatio);
        ...
        const remainingQuantity =
            quantity - opposingPositions.reduce((acc, pos) => acc + Math.min(quantity, pos.quantity), 0);

        // If there's remaining quantity, create a new position (isolated margin - no merging)
        if (remainingQuantity > 0) {
            const effectiveCollateral = base_currency_amount * (remainingQuantity / quantity);
            ...
            const newPosition = new Position({
                ...
                manual_symbol,
                base_currency,
                base_currency_amount: effectiveCollateral,
            ...
            await User.updateBalance(user._id, base_currency, -effectiveCollateral);
            ...
            // All quantity was used for reversals
            return res.json({
                success: true,
        ...
        // If no existing positions or no reversals, create a new position
        // Calculate isolated liquidation price
        const isolatedLiquidationPrice = calculateIsolatedLiquidationPrice(
            type as "buy" | "sell",
            entry_price,
...
export const adminCreatePosition = async (req: Request, res: Response) => {
    try {
        ...
        // Check for leverage mismatch if creating an open position
        if (status === "open" && leverage) {
            const existingPositions = await Position.collection
                .find({
            ...
            const sameSymbolTypePositions = existingPositions.filter(
                (pos) => pos.symbol === symbol && pos.type === type
            );

            if (sameSymbolTypePositions.length > 0) {
                const differentLeveragePosition = sameSymbolTypePositions.find((pos) => pos.leverage !== leverage);
                if (differentLeveragePosition) {
                    return res.status(400).json({
                        success: false,
        ...
        if (Number(exit_price) && status === "closed") {
            const pnl = calculatePnl(type, entry_price, exit_price, quantity, leverage || 1);
        ...
        if (status === "open" && leverage && !liquidation_price) {
        ...
        const position = new Position({
            ...
            type,
            leverage,
            liquidation_price: calculatedLiquidationPrice || liquidation_price || undefined,
            position_size,
            take_profit,
            stop_loss,
            fees: fees > 0 ? fees : undefined,
        ...
        if (Number(exit_price)) {
            const pnl = calculatePnl(type, entry_price, exit_price, quantity, leverage || 1);
            // CRITICAL FIX: When admin creates a closed position, return margin + PnL - fees
            // Not just PnL - fees
            await User.updateBalance(new ObjectId(user_id as string), base_currency, base_currency_amount + pnl - fees);
        } else {
        ...
        // Add to liquidation monitor tracking if open
        if (status === "open") {
...
export const adminUpdatePosition = async (req: Request, res: Response) => {
    try {
        ...
        // Calculate isolated liquidation price for open positions
        if (payload.leverage && !payload.liquidation_price && payload.status === "open") {
        ...
        if (Number(payload.exit_price)) {
            ...
            await User.updateBalance(user._id, payload.base_currency, payload.base_currency_amount + pnl - fees);
        } else {
