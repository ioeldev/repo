...
const TradingFormContext = createContext<TradingFormContextType | undefined>(
  undefined,
);
...
export const TradingFormProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const currentPrice =
    tickerData?.price || (selectedPair ? parseFloat(selectedPair.price) : 0);
  // Position size = margin required × leverage ÷ current price
  const positionSize =
    currentPrice > 0 ? (marginRequired * state.leverage) / currentPrice : 0;
  ...
  return (
    <TradingFormContext.Provider value={value}>
      {children}
