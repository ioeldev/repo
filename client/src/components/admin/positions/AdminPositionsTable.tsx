...
const RealTimePnLCell = memo(({ position, pairsPrice }: { position: AdminPosition; pairsPrice?: number }) => {
    ...
    return (
        ...
        >
            ...
            {hasLivePrice && <span className="text-xs ml-1 opacity-70">(Live)</span>}
        </div>
    );
});
...
const CurrentPriceCell = memo(({ position, pairsPrice }: { position: AdminPosition; pairsPrice?: number }) => {
    const symbol = `${position.symbol}${position.base_currency}`;
    const ticker = useTickerSymbol(symbol);
...
export function AdminPositionsTable({
    ...
    const ClosePositionDialog = memo(() => {
        const symbol = selectedPosition ? `${selectedPosition.symbol}${selectedPosition.base_currency}` : "";
        const ticker = useTickerSymbol(symbol);
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3 font-medium">
                            {selectedPosition.user.first_name} {selectedPosition.user.last_name}
                        </div>
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
        ...
        return (
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Êtes-vous sûr de vouloir supprimer cette position ? Cette action est irréversible.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
    ...
    const columns: ColumnDef<AdminPosition>[] = React.useMemo(
        () => [
            ...
            },
            {
                ...
                header: "Prix actuel",
                cell: ({ row }) => {
                    const pairsPrice = getPairPrice(row.original.symbol, row.original.base_currency);
                    return <CurrentPriceCell position={row.original} pairsPrice={pairsPrice} />;
                },
            },
            {
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    const pairsPrice = getPairPrice(row.original.symbol, row.original.base_currency);
                    return <RealTimePnLCell position={row.original} pairsPrice={pairsPrice} />;
                },
            },
            {
                accessorKey: "status",
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                {position.status === "open" && onClosePosition && (
                                    <DropdownMenuItem onClick={() => handleOpenCloseDialog(position)}>
        ...
        [formatAmount, onClosePosition, onDeletePosition, onEditPosition, getPairPrice]
    );
    ...
    const table = useReactTable({
        ...
        pageCount: totalRows ? Math.ceil(totalRows / (pagination?.pageSize || 10)) : -1,
        state: {
            sorting,
            columnFilters,
            globalFilter,
            ...
            {pagination && onPaginationChange && (
                <div className="flex items-center justify-between">
                    <div className="text-sm text-muted-foreground">
                        ...
                        {Math.min((pagination.pageIndex + 1) * pagination.pageSize, totalRows)} sur {totalRows}{" "}
                        positions
                    </div>
                    <div className="flex items-center gap-2">
                        <Button
                        ...
                        <div className="text-sm">
                            Page {pagination.pageIndex + 1} sur {Math.ceil(totalRows / pagination.pageSize) || 1}
                        </div>
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onPaginationChange({ ...pagination, pageIndex: pagination.pageIndex + 1 })}
                            disabled={pagination.pageIndex >= Math.ceil(totalRows / pagination.pageSize) - 1}
                        >
                            Suivant
                        </Button>
                        <Button
